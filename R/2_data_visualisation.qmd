...
# En voulant reprendre les data.frame, déja transformé, j'avais des problèmes de classes d'objet... d'ou ce nouveau snv_x
...
Test de l'erreur de prédiction avec l'utilisation de fonctions splines

```{r}
require(splines)
#Création d'une matrice contenant tous les spectres
snv_x <- t(scale(t(data_NIRS_raw[,-c(1:14)])))

#Définition des noeuds (ici 50 noeuds entre le min et le max de wavelength)
knots = seq(min(wavelength),max(wavelength),length=50)

# Création des splines de base
base <- bs(wavelength, knots=knots[-c(1,length(knots))])

#Création des splines pour tous spectres et extraction des coefficients
res <- apply(snv_x[1:2427, ], 1, function(row) {
  #création d'un data.frame en fusionnant une ligne (=1 spectre) de snv_x et les 
  #B splines
  df <- cbind.data.frame(x = row, base)
  #Modèle linéaire entre un spectre et les B splines
  mod <- lm(x ~ ., data = df)
  #Extraction des coefficients de la fonction spline
  coef(mod)
})

#Fusion de "res" avec une variable explicative ici "N content"
res <- t(res)
res <- cbind.data.frame(data_NIRS_raw[1:2427, 9], res[,-1])

#Création du modèle linéaire
mod <- lm(`N.content...DM.`~., data = data.frame(res))
F_mod <- summary(mod)$fstatistic[[1]]
F_mod
...
segments = cvsegments(n,10)  
#Création d'un vecteur vide pour acceuillir les valeur prédites du modèle
cvpred = rep(0,n) 

#Pour tous les segments
for (k in 1:10) {
   #Ajustement du modèle sur tous les segments sauf le kième
   dta = lm(`N.content...DM.`~.,data=res[-segments[[k]],])
   #Prediction avec le modèle sur le segment k
   cvpred[segments[[k]]] = predict(dta,newdata=res[segments[[k]],])
}

#Calcul du PRESS du modèle
PRESS = sum((res$`N.content...DM.`-cvpred)^2)
```
